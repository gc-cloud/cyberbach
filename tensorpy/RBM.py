...
def gibbs_sample(x, W, bv, bh, k):
    ...
    # Run gibbs steps for k iterations ct is the counter
    ct = tf.constant(0)
    ...
    # We need this  to stop tensorflow from propagating gradients back through the gibbs step
    x_sample = tf.stop_gradient(x_sample)
    return x_sample

def get_free_energy_cost(x, W, bv, bh, k):   
    # We use this function in training to get the free energy cost of the RBM. We can pass this cost directly
    # into TensorFlow's optimizers
...
def get_cd_update(x, W, bv, bh, k, lr):
    ...
    # Next, we update the values of W, bh, and bv, based on the difference between the samples that we drew and
    # the original values
